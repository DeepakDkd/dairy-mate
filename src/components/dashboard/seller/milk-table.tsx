...
export function SellerMilkTable({
  ...
  const exportUrl =
    selectedDairyId &&
    `/api/dairies/${selectedDairyId}/exports?report=seller-entries&month=${month}`
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      console.error("Failed to export seller entries:", error)
                      ...
                      <TableRow key={entry.id} className="hover:bg-secondary/50">
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-2">
                            <Button variant="outline" size="sm" onClick={() => openEditDialog(entry)} disabled={isMonthClosed}>
